...
class WriteFile(ABC):
    """Абстрактный класс работы с экземпляром самолета"""

    @abstractmethod
    def add_airplane(self, plane: PlanesInfo) -> None:
        """Абстрактный метод записи в файл обновленного списка словарей с информацией о самолетах"""
        pass
    ...
    @abstractmethod
    def del_airplane(self, plane: PlanesInfo) -> None:
        """Абстрактный метод удаления информации о самолетах из json файла"""
        pass
...
class WriteFileJson(WriteFile):
    ...
    def get_dict_airplanes(self, plane: PlanesInfo) -> dict:
